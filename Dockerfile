...
RUN mkdir /home/user && \
    ...
    ncurses pkgconf-pkg-config \
    # developer tools
    git wget tar \
    # install yq
    python2-pip python2-pip-wheel \
    && pip2 install yq && \
    cd /tmp && wget http://mirror.centos.org/centos/8/BaseOS/x86_64/os/Packages/bash-completion-2.7-5.el8.noarch.rpm && \
    rpm --install bash-completion-2.7-5.el8.noarch.rpm && \
    rm bash-completion-2.7-5.el8.noarch.rpm && \
